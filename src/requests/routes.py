...
async def create_update_book_request(book_id: int,update_data: BookCreateModel, current_user: User = Depends(get_current_user), session : AsyncSession = Depends(get_session)):
    ...
    await _service.create_update_book_request(book_id, update_data, current_user)    

@requests_router.get("/check_requests")
async def check_request(session: AsyncSession = Depends(get_session)):
    _service = RequestService(session)
    await _service.check_requests()