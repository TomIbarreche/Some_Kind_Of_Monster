...
async def create_update_book_request(book_id: int,update_data: BookCreateModel, current_user: User = Depends(get_current_user), session : Session = Depends(get_db)):
...
async def check_request_from_mail(token:str, session: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    _service = RequestService(session)
...
async def get_all_user_request(user_id: int, session: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    _service = RequestService(session)
...
async def get_all_requests(limit: Annotated[int, Query(gt=0,le=100)] = 10, offset: Annotated[int, Query(gt=-1, le=100)]=0, session: Session = Depends(get_db)):
    _service = RequestService(session)
...
async def validate_request(request_id: int, session: Session = Depends(get_db), current_user:User = Depends(get_current_user)):
    _service = RequestService(session)
...
async def update_request(request_id: int, request_data: UpdateRequest, session: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    _service = RequestService(session)
