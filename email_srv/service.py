...
class MailService:
    ...
    def create_body(self, subject:str, token:str) -> str:
        if subject == settings.update_request_mail_subject:
            ...
            """
        elif subject == settings.validate_request_mail_subject:
            link = f"http://{settings.domain_name}{settings.api_prefix}/books/{token}"
            body = f"""
            <h1>Your request have been validated</h1>
            <p>Follow this <a href={link}>link</a> to see the update</p>
            """
        else:
